...
ggplot(nobel_top5c_count, 
  ...
  theme_cavis_hgrid +     
  facet_wrap(~ category) +                         
  geom_vline(xintercept = 1950, linetype = 2, color = "grey45") + #BREAK8
  geom_line() +   #BREAK2
  scale_color_brewer(type = "qual", palette = 3) + #BREAK3
  theme(legend.position = "top") +                 #BREAK4
  geom_point(color = "white", size = 4) + #BREAK6
  geom_point(size = 2) + #BREAK5
  guides(colour = guide_legend(reverse = TRUE)) + #BREAK7 
  labs(x = "Decade", y = "Total number of awards (per decade)") #BREAK9
```
...
nobel2 %>%
  ...
  geom_smooth(method = "gam", color = "grey45",
              alpha = 0.25, size = 0.5) +
...
ggplot(cyYoungFD,
  ...
  geom_text(data = filter(cyYoungFD, covariate == "era"), 
            aes(label = c("Model 1", "Model 2")),
            position = position_dodge2v(height = 1.25)) +
